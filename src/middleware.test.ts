...
describe("middleware", () => {
  ...
  it("preserves /award-information in returnTo when route is protected", async () => {
    mockGetUser.mockResolvedValue({ data: { user: null }, error: null });
    const { middleware } = await import("./middleware");
    await middleware(makeRequest("/award-information"));

    expect(NextResponse.redirect).toHaveBeenCalledWith(
      expect.objectContaining({
        pathname: "/login",
        search: "?returnTo=%2Faward-information",
      }),
    );
  });
  ...
  describe("/countdown route", () => {
    ...
    it("TC-C2: passes through when event date is in the future", async () => {
      process.env.NEXT_PUBLIC_EVENT_START_DATE = new Date(
